...
def main():
    ...
    parser.add_argument('-m', '--model', default='models/depth_anything_v2.dxnn', help='Path to .dxnn model')
    parser.add_argument('-s', '--side', action='store_true', help='Show side by side')
    parser.add_argument('-g', '--grayscale', action='store_true', help='Use grayscale colormap')
    parser.add_argument(
        '--bg-color',
        default='0,0,0',
            ...
            if not result_queue.empty():
                ...
                if args.side:
                    if orig.shape[:2] != depth_colored.shape[:2]:
                        orig = cv2.resize(orig, (depth_colored.shape[1], depth_colored.shape[0]))
