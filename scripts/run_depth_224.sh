...
if [ "$DX_BACKEND" == "python" ]; then
    ...
    if [ -d "python" ]; then
        ...
        if [ -n "$(find . -maxdepth 2 -name '*.py' -not -name '__init__.py' | grep -i 'main\|demo\|gui' | head -n 1)" ]; then
            ...
            source "${ROOT_DIR}"/.venv/bin/activate && python "$py_file" --backend dxnn --model "${WORKSPACE}/models/depth/depth_anything_v2_vits_224x224.dxnn" --camera "${DX_CAMERA_IDX:-0}" --exit-btn --full_screen
    ...
    cd cpp
    ./build/depth-demo -m "${WORKSPACE}/models/depth/depth_anything_v2_vits_224x224.dxnn" -s --exit-btn -c "${DX_CAMERA_IDX:-0}"
    #./build/depth-demo -m "${WORKSPACE}/models/depth/depth_anything_v2_vits_224x224.dxnn" -s
