/**
 * @file async_semantic_seg_runner.hpp
 * @brief Asynchronous semantic segmentation runner using factory pattern
 *
 * Provides a generic async runner that accepts any ISegmentationFactory implementation.
 * Note: Segmentation factories do NOT take is_ort_configured parameter.
 */

#ifndef ASYNC_SEMANTIC_SEG_RUNNER_HPP
#define ASYNC_SEMANTIC_SEG_RUNNER_HPP
...
struct AsyncSemanticSegDisplayArgs {
    std::shared_ptr<std::vector<SegmentationResult>> results;
    ...
    uint64_t frame_id = 0;
    AsyncSemanticSegDisplayArgs() = default;
    AsyncSemanticSegDisplayArgs(const AsyncSemanticSegDisplayArgs&) = default;
    AsyncSemanticSegDisplayArgs& operator=(const AsyncSemanticSegDisplayArgs&) = default;
    AsyncSemanticSegDisplayArgs(AsyncSemanticSegDisplayArgs&&) noexcept = default;
    AsyncSemanticSegDisplayArgs& operator=(AsyncSemanticSegDisplayArgs&&) noexcept = default;
};

template <typename FactoryT>
class AsyncSemanticSegRunner {
    bool verbose_ = false;

public:
    explicit AsyncSemanticSegRunner(std::unique_ptr<FactoryT> factory)
    ...
    int run(int argc, char* argv[]) {
        ...
        if (args.loop_forever) {
            std::cout << "[INFO] --loop: input will repeat indefinitely." << std::endl;
        }
        ...
        int last_job_id = -1;
        uint64_t next_submit_frame_id = 0;
        ...
        auto processFrameAsync = [&](const cv::Mat& frame) {
            ...
            user_data_ptr->frame_id = next_submit_frame_id++;
            traceFrameOrder("submit", user_data_ptr->frame_id);
        ...
        if (is_image) {
            ...
            for (int i = 0; (args.loop_forever || i < loopTest) && running_ && !g_interrupted(); ++i) {
                ...
                ud->frame_id = next_submit_frame_id++;
                traceFrameOrder("submit", ud->frame_id);
                metrics_.waitForSlot();
                {
                    std::lock_guard<std::mutex> lock(metrics_.metrics_mutex);
                    auto now = std::chrono::high_resolution_clock::now();
            ...
            for (int loop_idx = 0; (args.loop_forever || loop_idx < loopTest) && running_ && !g_interrupted(); ++loop_idx) {
                if (args.loop_forever && loop_idx > 0) {
                    if (args.verbose) {
                        std::cout << "\n" << std::string(50, '=') << std::endl;
                        std::cout << "[INFO] Loop " << (loop_idx + 1) << " (infinite)" << std::endl;
                ...
                if (args.videoFile.empty()) continue;
                if (!args.loop_forever && loop_idx + 1 >= loopTest) continue;
                video.release();
...
private:
    ...
    std::string frame_order_trace_path_;
    std::mutex frame_order_trace_mutex_;

    /** Handle async inference completion for semantic segmentation. */
    int onAsyncInferenceComplete(
        ...
        {
            std::lock_guard<std::mutex> lock(metrics_.metrics_mutex);
            double t_inf = std::chrono::duration<double, std::milli>(t_callback_start - ud_ptr->submit_ts).count();
        ...
        display_args.results = std::make_shared<std::vector<SegmentationResult>>(std::move(results));
        display_args.t_postprocess = t_postprocess;
        display_args.ctx = ud_ptr->ctx;
        display_args.save_path = ud_ptr->save_path;
        display_args.frame_id = ud_ptr->frame_id;
        traceFrameOrder("callback", display_args.frame_id);

        verify::dumpVerifyJson(*display_args.results, model_path_,
            "semantic_segmentation", display_args.original_frame->rows, display_args.original_frame->cols);
    ...
    CommandLineArgs parseCommandLine(int argc, char* argv[]) {
        ...
        options.add_options()
            ...
            ("l, loop", "Loop input indefinitely (video/camera/RTSP/image)",
             cxxopts::value<bool>(args.loop_forever)->default_value("false")->implicit_value("true"))
            ...
            ("full_screen", "Show output in fullscreen",
             cxxopts::value<bool>(args.full_screen)->default_value("false"))
    ...
    void traceFrameOrder(const char* stage, uint64_t frame_id) {
        if (frame_order_trace_path_.empty()) return;
        std::lock_guard<std::mutex> lock(frame_order_trace_mutex_);
        std::ofstream out(frame_order_trace_path_, std::ios::app);
        if (out.is_open()) out << stage << "," << frame_id << "\n";
    }

    void displayThread(IVisualizer<SegmentationResult>& visualizer, bool no_display,
                       bool save_on, cv::VideoWriter& writer) {
        std::map<uint64_t, AsyncSemanticSegDisplayArgs> pending;
        uint64_t next_display_frame_id = 0;

        auto renderOne = [&](AsyncSemanticSegDisplayArgs& args) {
        ...
        while (running_ || !display_queue_.empty() || !pending.empty()) {
            AsyncSemanticSegDisplayArgs args;
            if (display_queue_.try_pop(args, std::chrono::milliseconds(100))) {
                pending.emplace(args.frame_id, std::move(args));
            }
            while (true) {
                auto it = pending.find(next_display_frame_id);
                if (it == pending.end()) break;
                traceFrameOrder("render", it->second.frame_id);
                renderOne(it->second);
                pending.erase(it);
                ++next_display_frame_id;
            }

            // On shutdown, do not wait forever if a frame was dropped or never reached
            // the display queue. Flush remaining completed frames in ascending frame_id
            // order so the worker can terminate cleanly.
            if (!running_ && display_queue_.empty() && !pending.empty() &&
                pending.find(next_display_frame_id) == pending.end()) {
                auto it = pending.begin();
                next_display_frame_id = it->first;
                traceFrameOrder("render", it->second.frame_id);
                renderOne(it->second);
                pending.erase(it);
                ++next_display_frame_id;
...
#endif  // ASYNC_SEMANTIC_SEG_RUNNER_HPP
