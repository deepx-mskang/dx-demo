...
public:
    ...
    int run(int argc, char* argv[]) {
        ...
        if (is_image) {
            processImageFrames(imageFiles, loopTest, args.loop_forever, display_image,
            ...
            processVideoFrames(video, display_image,
                               ie, *preprocessor, *postprocessor, *visualizer, metrics,
                               processCount, writer, args.no_display, args.saveMode,
                               loopTest, args.loop_forever, args.videoFile,
                               dumpTensorsBaseDir);
        }
...
private:
    ...
    void processImageFrames(
        const std::vector<std::string>& imageFiles, int loopTest, bool loop_forever,
        ...
        for (int i = 0; (loop_forever || i < loopTest) && !g_interrupted().load(); ++i) {
    ...
    void processVideoFrames(
        ...
        IVisualizer<SegmentationResult>& visualizer, SyncProfilingMetrics& metrics,
        int& processCount, cv::VideoWriter& writer, bool no_display, bool saveMode,
        int loopTest, bool loop_forever, const std::string& videoFile,
        const std::string& dumpTensorsBaseDir = "") {
        for (int loop_idx = 0; (loop_forever || loop_idx < loopTest) && !g_interrupted().load(); ++loop_idx) {
            if (loop_forever && loop_idx > 0) {
                if (verbose_) {
            ...
            if (!loop_forever && loop_idx + 1 >= loopTest) continue;
            video.release();
