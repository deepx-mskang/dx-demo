...
MODELS_DIR="$PROJECT_ROOT/engine/model_files"
DXNN_DIR="$MODELS_DIR/server"
DXNN_MOBILE_DIR="$MODELS_DIR/mobile"
...
verify_dataset() {
    ...
    if [[ $image_count -lt 10 ]]; then
        log "WARNING: Only $image_count ../../workspace/models/ocr/v6/images found. Please ensure image files are copied from C++ baseline"
    else
        log "✓ Dataset ready: $image_count ../../workspace/models/ocr/v6/images found"
    fi
...
# Ask user whether to run benchmark; wait indefinitely for user input
USER_INPUT=""
read -p "Run benchmark now? [y/N]: " USER_INPUT
if [[ ! "$USER_INPUT" =~ ^[Yy]$ ]]; then
    log "Benchmark skipped by user."
...
run_sync_benchmark() {
    ...
    local benchmark_cmd="python scripts/dxnn_benchmark.py --directory ../../workspace/models/ocr/v6/images --ground-truth ../../workspace/models/ocr/v6/images/labels.json --output output_sync --runs 1 --save-individual --mode sync"
...
run_async_benchmark() {
    ...
    local benchmark_cmd="python scripts/dxnn_benchmark.py --directory ../../workspace/models/ocr/v6/images --ground-truth ../../workspace/models/ocr/v6/images/labels.json --output output_async --runs 1 --save-individual --mode async"
...
# check dxrt.service is running 
if ! systemctl is-active --quiet dxrt.service; then
    log "ERROR: dxrt.service is not running. Please start it with 'sudo systemctl start dxrt.service'"
    sudo systemctl status dxrt.service
    sudo systemctl restart dxrt.service
    log "Restarted dxrt.service, please try running the benchmark again."
fi
