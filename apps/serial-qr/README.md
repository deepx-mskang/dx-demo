...
2. **인식** — 라벨을 화면의 점선 가이드 안에 맞추고 `시리얼 인식` 을 누릅니다
   (스페이스바도 동일). 검출/인식 지연이 ms 단위로 표시됩니다.
3. **발행** — `QR 생성 →` 을 누르면 QR 과 기기 정보가 나옵니다.
4. **조회** — 휴대폰 기본 카메라로 QR 을 비추면 기기 정보 페이지가 열립니다.
   **휴대폰이 데모 PC 와 같은 네트워크에 있어야 합니다.**

라벨이 없을 때는 스캔 화면 하단의 `라벨 없이 시연하기` 에서 시리얼을 직접 골라
2~4 단계만 시연할 수 있습니다.
...
| `POST` | `/api/scan` | 최신 프레임 OCR → 시리얼 추출 |
...
| 시리얼을 못 읽음 | 라벨을 가이드 안에 크게, 정면으로. `OCR 원본 텍스트` 를 펼쳐 무엇이 읽혔는지 확인 |
